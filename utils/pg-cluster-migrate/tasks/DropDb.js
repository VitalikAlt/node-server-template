...
class DropDb {
    ...
    run() {
        return new Promise((resolve, reject) => {
            ...
            this.reject = reject;
            this.pgShard.sqlQuery("SELECT 1 as test from pg_database WHERE datname='" + this.params.database + "'").then(this.onSuccessTest.bind(this), this.onErrorTest.bind(this))
        })
    }
    ...
    onSuccessTest(result) {
        if (result.length > 0) {
            ...
            this.pgShard.sqlQuery('DROP DATABASE IF EXISTS ' + this.params.database).then(this.successDrop.bind(this), this.errorDrop.bind(this))
        } else {
